...
function formatTime(minutes, seconds) {
  const arr = ['0', String(minutes), ':', String(seconds)];
  if (seconds === 0) {
    arr.push('0');
  }
  return arr.join('');
...
function countVowels(str) {
  let counter = 0;
  const vowals = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  const arr = str.split('');
  for (let i = 0; i < arr.length; i += 1) {
    for (let k = 0; k < vowals.length; k += 1) {
      if (arr[i] === vowals[k]) {
        counter += 1;
      }
    }
...
function isPalindrome(str) {
  const resStr = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  return resStr === resStr.split('').reverse().join('');
}
